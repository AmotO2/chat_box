...
class ChatClient:
    def __init__(self, ip, port, update_ui_callback):
        self.client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.client_socket.connect((ip, port))
        self.update_ui_callback = update_ui_callback

    def send_group_message(self, message, group):
        data = {"type": "group_message", "data": message, "group_name": group}
    ...
    def send_file(self, file_path, group):
        ...
        self.client_socket.send(pickle.dumps({"type": "file_transfer", "group_name": group, "filename": filename}))
        self.client_socket.send(file_size.to_bytes(8, 'big'))  # Send file size
        self.client_socket.sendall(file_data)  # Send file data

    def create_group(self, group_name):
        data = {"type": "create_group", "group_name": group_name}
        self.client_socket.send(pickle.dumps(data))

    def join_group(self, group_name):
        data = {"type": "join_group", "group_name": group_name}
        self.client_socket.send(pickle.dumps(data))
        ...
        while True:
            try:
                ...
                if data:
                    ...
                    # Update the UI
                        self.update_ui_callback(f"File received from {group_name}: {filename}")
...
def main():
    ...
    client = ChatClient('127.0.0.1', 9999, window.display_message)
